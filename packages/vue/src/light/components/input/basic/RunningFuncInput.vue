...
import FuncInputVue from '../../constant/basic/FuncInput.vue';

const unwrapValue = (v: CandidFunc): CandidFuncValue => v.value;
const wrapValue = (v: CandidFuncValue): CandidFunc => {
    return { type: 'func', value: v };
};
...
const props = defineProps({
    ...
    initial: {
        type: Object as PropType<FuncCandidType>,
        required: true,
    },
    ui: {
...
const current = ref<CandidFunc>(
    readRuntime<CandidFunc>(props.initial)?.ok ??
        (getInitialCandidTypeValue(props.initial, [], []) as CandidFunc),
);

const label = ref(props.ui?.label ?? '');
const hasDefaultValue = ref<boolean>(!!props.ui?.default);
const defaultValue = ref<CandidFunc>(
    getInitialCandidTypeValue(props.initial, [], []) as CandidFunc,
);

let currentResult: DataResult<CandidFunc> =
    current.value !== undefined
        ? { ok: current.value }
        : { err: { message: `value can not be undefined` } };
let labelResult: DataResult<string> = { ok: label.value };
let defaultValueResult: DataResult<CandidFunc> = { ok: defaultValue.value };

const parsedCurrentValue = computed(() => unwrapValue(current.value));
...
const init = () => {
    ...
    const newDefaultValue = getInitialCandidTypeValue(props.initial, [], []) as CandidFunc;
    ...
    const newDefaultValueResult = newDefaultValue.value
        ...
        : {
              err: {
                  message: 'default func value can not be empty',
...
const onCurrentChanged = (r: DataResult<CandidFuncValue>) => {
...
const onDefaultValueChanged = (r: DataResult<CandidFuncValue>) => {
    const rr = !r.err ? { ok: wrapValue(r.ok) } : { err: r.err };
...
const produce = (): DataResult<FuncCandidType> => {
    if (!initialed) return { err: { message: `${props.status} func input has not been initial.` } };
...
const emit = defineEmits<{
    changed: [DataResult<FuncCandidType>];
}>();
...
<template>
    <div class="running-func-input-content">
        <div class="func-main">
            <div class="main-content">
                ...
                <div class="func-content">
                    <FuncInputVue :initial="parsedCurrentValue" @changed="onCurrentChanged" />
                </div>
            </div>
