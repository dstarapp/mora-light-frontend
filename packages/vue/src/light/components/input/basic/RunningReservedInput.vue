...
import { ReservedCandidType } from '@mora-light/core/types/candid';
import {
    checkAndAssignValue,
    checkAndExecute,
    DataResult,
...
const props = defineProps({
    ...
    initial: {
        type: Object as PropType<ReservedCandidType>,
        required: true,
    },
    ui: {
...
const produce = (): DataResult<ReservedCandidType> => {
    if (!initialed)
        return { err: { message: `${props.status} reserved input has not been initial.` } };
...
const emit = defineEmits<{
    changed: [DataResult<ReservedCandidType>];
}>();
...
<template>
    <div class="running-reserved-input-content">
...
.running-reserved-input-content {
    width: 100%;
    display: flex;
