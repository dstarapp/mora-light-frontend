...
import { EmptyCandidType } from '@mora-light/core/types/candid';
...
const props = defineProps({
    ...
    initial: {
        type: Object as PropType<EmptyCandidType>,
        required: true,
    },
    ui: {
...
const produce = (): DataResult<EmptyCandidType> => {
    if (!initialed)
        return { err: { message: `${props.status} empty input has not been initial.` } };
...
const emit = defineEmits<{
    changed: [DataResult<EmptyCandidType>];
}>();
...
<template>
    <div class="running-empty-input-content">
