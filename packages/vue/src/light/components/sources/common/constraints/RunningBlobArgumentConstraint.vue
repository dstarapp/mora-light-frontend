...
const calcInnerValues = (
    ...
    blobSubitems: ArrayArgumentConstraintSubitem[],
    ...
    const length = Math.max(wrappedConstant, blobSubitems.length);
    if (length > 0) {
        for (let i = 1; i < length; i++) {
            const c = wrappedSubitems[i - 1] ?? blobSubitems[i].constraint ?? defaultConstraint; // Get the previous data source
...
const blobLength = ref<DataSource | undefined>(props.initial.length);
const blobSubitems = ref<ArrayArgumentConstraintSubitem[] | undefined>(props.initial.subitems);

const wrappedSubitems = ref<ArgumentConstraint[]>(props.initial.subitems2 ?? []);
const blobSubitemsRuntime = ref<(number | undefined)[]>([]);

// Receive data that is passed upward
let { lengthResult: blobLengthResult, subitemsResults: blobSubitemsResults } = parseDataResult(
    blobConstant.value,
    blobLength.value,
    blobSubitems.value,
...
const wrappedConstant = computed(() => {
    ...
    const runtime = readRuntime<number>(
        blobLength.value ? findDataSourceType(blobLength.value) : undefined,
    );
...
const checkWrappedSubitems = () => {
    ...
    if (blobSubitems.value === undefined) return;
    ...
    while (blobSubitemsRuntime.value.length > length)
        blobSubitemsRuntime.value.splice(subitems.length - 1, 1);

    for (let i = 0; i < length; i++) {
        if (subitems[i] !== undefined) continue;
        const subitem = blobSubitems.value[i];
        const constraint = subitem?.constraint ?? deepClone(props.initial.default);
        const runtime = blobSubitemsRuntime.value[i];
...
watch(
    () => [wrappedConstant.value, blobSubitems.value, props.initial.default],
    (nv, ov) => {
...
const innerValues = computed<ValueItem[][]>(() =>
    blobLength.value === undefined ||
    blobSubitems.value === undefined ||
...
const init = () => {
    ...
    const newBlobSubitems = props.initial.subitems;

    const { lengthResult: newBlobLengthResult, subitemsResults: newBlobSubitemsResults } =
        parseDataResult(
            newBlobConstant,
            newBlobLength,
            newBlobSubitems,
    ...
    let newBlobSubitemsRuntime = blobSubitemsRuntime.value;
    if (!isSameCandidType(checkedType, newCheckedType)) newBlobSubitemsRuntime = [];
    ...
    if (
        ...
        same(blobSubitems.value, newBlobSubitems) &&
        same(blobLengthResult, newBlobLengthResult) &&
        same(blobSubitemsResults, newBlobSubitemsResults) &&
        same(blobSubitemsRuntime.value, newBlobSubitemsRuntime)
    ) {
    ...
    blobSubitems.value = newBlobSubitems;

    blobLengthResult = newBlobLengthResult;
    blobSubitemsResults = newBlobSubitemsResults;

    blobSubitemsRuntime.value = newBlobSubitemsRuntime;
...
const onBlobLengthChanged = (r: DataResult<DataSource>) => {
    blobLengthResult = r;

    if (r.ok !== undefined) blobLength.value = r.ok;

    changed();
};

const onBlobSubitemsChanged = (i: number, r: DataResult<ArgumentConstraint>) => {
    if (blobSubitems.value === undefined) return;
    if (blobSubitemsResults === undefined) return;

    const item = blobSubitems.value[i];

    if (item?.constraint !== undefined) {
        // It is customized
        blobSubitemsResults[i] = r.err
            ? { err: r.err }
            : { ok: { type: 'custom', constraint: r.ok } };

        if (r.ok !== undefined) blobSubitems.value[i] = { type: 'custom', constraint: r.ok };
    }
    ...
    if (r.ok !== undefined) {
        ...
        if (runtime?.ok !== undefined)
            blobSubitemsRuntime.value[i] = runtime.ok; // In case it is worth recording
        else blobSubitemsRuntime.value[i] = undefined;
    } else {
        blobSubitemsRuntime.value[i] = undefined;
    }
...
const produce = (): DataResult<BlobArgumentConstraint> => {
    if (!initialed)
        return {
            err: { message: `${props.status} blob argument constraint has not been initial.` },
        };
    ...
    if (blobLengthResult?.ok) ok.length = blobLengthResult.ok;
    ...
    if (blobSubitemsResults) {
        for (let i = 0; i < blobSubitemsResults.length; i++) {
            const err = blobSubitemsResults[i].err;
            if (err) return { err };
        }

        ok['items'] = blobSubitemsResults.map((r) => r.ok!);
    ...
    if (wrappedConstant.value > 0) {
        ...
        for (let i = 0; i < wrappedConstant.value; i++) {
            const value = blobSubitemsRuntime.value[i];
            if (value === undefined) {
                runtime = undefined;
                break;
            }
...
<template>
    <div class="running-blob-argument-constraint-content">
        <template v-if="blobLength">
            <RunningWrappedSourceVue
                ...
                @changed="onBlobLengthChanged"
                ref="lengthRef"
                :blob-length="blobLength.source"
            />
        </template>

        <div
            class="tip"
            v-if="props.status === 'using' && blobLength && hasUIByDataSource(blobLength)"
        >
            List
        </div>
        <div
            ...
            :class="{
                'margin-top': blobLength && hasUIByDataSource(blobLength),
            }"
        >
            <div class="item" v-for="(subitem, i) in wrappedSubitems" :key="i" :blob-index="i">
                <span>{{ `${i}` }}</span>
                ...
                <RunningArgumentConstraintVue
                    ...
                    :calling="props.calling"
                    :canExportValues="canExportValues && blobConstant > 0"
                    :index="i"
                    :recItems="props.recItems"
                    :initial="subitem"
                    :hasLabel="props.hasLabel"
                    :triggerRefresh="props.triggerRefresh"
                    @changed="(r) => onBlobSubitemsChanged(i, r)"
...
.running-blob-argument-constraint-content {
    width: 100%;
    > .tip {
        width: 100%;
        height: 28px;
